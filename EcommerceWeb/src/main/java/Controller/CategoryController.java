...
public class CategoryController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		System.out.println(req.getRequestURI().substring("/EcommerceWeb/Categories/".length()+1));

        if (req.getRequestURI().substring("/EcommerceWeb/Categories/".length()+1).equals("all")) {
            try {
    		...
    		Long id=Long.parseLong(req.getRequestURI().substring("/EcommerceWeb/Categories/".length()));
